...
@ varargin{1} =[active_L x1]
@ varargin{2} = params :
        for fitting ellipse:  params = [P Q theta semn]
        for fitting cylinder: params = []
...
xc_offset = output.active_x(1,1);
x2 = output.active_x(end,1);

if verbose
    fprintf('\n\n\n  +++++++++++++++++++++++++++++++++++++++++++ \n')
    fprintf('  vvvvvvvvvv  analyse1Ddata says:  vvvvvvvvvv \n\n')
    fprintf(text)
    fprintf('  *  left trim = %f mm; \n  *  right trim = %f mm; \n', (xc_offset - x(1,1))*10^3, (x(end,1)-x2)*10^3)
    fprintf('  *  x1 = %f mm; \n  *  x2 = %f mm; \n  *  active_L = %f mm;  \n', xc_offset*10^3,x2*10^3, (x2-xc_offset)*10^3 )  
...
%% do the fitting
params = varargin{2};
if ~isempty(varargin{2}) % if fitting to ellipse, centre x values around 0
...
% aa = output.active_x(1,1);
% bb = output.active_x(end,1);
% ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
%% fit and integrate
for kk = 1:size(x,2)
