function [fname, cale, data_type] = readSPIPasc(fname);
% ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
for kk = 1:numel(fname)
    ...
    a = textscan(config{4}, '%s %f', 'delimiter','=');
    Nx = a{2};
    ...
    if isempty(Nx)  % if the data a 1d profile
        ...
        cale = [cale(1:end-length('_function_library\functions\readSPIPasc'))   '_data\'];
        ...
        a = textscan(config{10}, '%s %s', 'delimiter','=');    
        unit = a{2};
...
%      
        ...
        if x_length>50*10^-6 %&& linenn > Npoints+linen % if not AFM and there is a mask, get mask
            mask = dlmread([pathname filename],'\t',Npoints+linen, 0);    
            mask(:,end) = [];
            mask(mask==1) = nan;
            mask = mask+1;
        ...
        cale = mfilename('fullpath');
        cale = [cale(1:end-length('_function_library\functions\readSPIPasc'))   '_data\_maps\'];
