...
function supersmith_OpeningFcn(hObject, eventdata, handles, varargin)
    ...
    % Display
    a = imread([handles.pathname 'supersmith.jpg']);
    axes(handles.axes1)
...
function pb_load_all_Callback(hObject, eventdata, handles)    
    ...
    if exist('flip_flag','var')   % 2nd revision
        ...
        if exist('datasets','var')     % 1st revision
            ...
            if exist('one','var')
                handles.mydata.datasets{1} = one;
                handles.mydata.set_names{1} = one.namelist{1};
                handles.mydata.active_sets = 1;
            end
            if exist('two','var')
                handles.mydata.datasets{2} = two;
                handles.mydata.set_names{2} = two.namelist{1};
                handles.mydata.active_sets = sort([handles.mydata.active_sets   2]);
            end
            if exist('three','var')
                handles.mydata.datasets{3} = three;
                handles.mydata.set_names{3} = three.namelist{1};
                handles.mydata.active_sets = sort([handles.mydata.active_sets   3]);
            end
            if exist('four','var')
                handles.mydata.datasets{4} = four;
                handles.mydata.set_names{4} = four.namelist{1};
                handles.mydata.active_sets = sort([handles.mydata.active_sets   4]);
            end
            if exist('five','var')
                handles.mydata.datasets{5} = five;
                handles.mydata.set_names{5} = five.namelist{1};
                handles.mydata.active_sets = sort([handles.mydata.active_sets   5]);
            end
            if exist('six','var')
                handles.mydata.datasets{6} = six;
                handles.mydata.set_names{6} = six.namelist{1};
                handles.mydata.active_sets = sort([handles.mydata.active_sets   6]);
            end
        end
...
function pb_load_dataset_from_workspace_Callback(hObject, eventdata, handles)    
    % load everything(!) from base workspace . pas de gardefou!! 
    ...
    if get(handles.cb_plot_figure,'Value')    
        ...
        for kk = 1:numel(handles.mydata.active_sets)
            if handles.mydata.plot_flag(handles.mydata.active_sets(kk))
                hold on
                if handles.mydata.fitted_flag(1,handles.mydata.active_sets(kk)) &&  get(handles.cb_plot_fitted, 'Value')                
...
% ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    
    if get(handles.cb_plot_figure_err,'Value')    
        ...
        for kk = 1:numel(handles.mydata.active_sets)
            if handles.mydata.plot_flag(handles.mydata.active_sets(kk))
                hold on
                if handles.mydata.fitted_flag(2,handles.mydata.active_sets(kk))  && get(handles.cb_plot_fitted, 'Value')
                    plot(handles.mydata.datasets{handles.mydata.active_sets(kk)}.active_x*10^3,...
                        handles.mydata.datasets{handles.mydata.active_sets(kk)}.fitted.height_err*10^9,...
                        'DisplayName',handles.mydata.set_names{handles.mydata.active_sets(kk)},'Color', handles.mydata.colormap(handles.mydata.active_sets(kk),:),   'LineWidth',2)
                elseif isfield(handles.mydata.datasets{handles.mydata.active_sets(kk)},'mavg') && get(handles.cb_plot_mavg, 'Value')
                    plot(handles.mydata.datasets{handles.mydata.active_sets(kk)}.active_x*10^3,...
...
% PLOT FREQUENCIES
% +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    if get(handles.cb_plot_frequencies,'Value')
        if ~ishandle(123)
            figure(123),   set(gcf,'color','w',  'Position', [200, 50, 800, 400]);
            set(gcf,'DefaultAxesColorOrder',handles.mydata.colormap);
            box on
        else
            figure(123)
...
% PLOT JOKER1
% +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     if get(handles.cb_joker1,'Value')    
            if ~ishandle(124)
                figure(124),   set(gcf,'color','w',  'Position', [200, 50, 800, 400]);  
                set(gcf,'DefaultAxesColorOrder',handles.mydata.colormap);
                box on
            else
                figure(124)
...
% PLOT JOKER2
% +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     if get(handles.cb_joker2,'Value')    
            if ~ishandle(125)
                figure(125),   set(gcf,'color','w',  'Position', [200, 50, 800, 400]);  
                set(gcf,'DefaultAxesColorOrder',handles.mydata.colormap);
                box on
            else
                figure(125)
...
function handles = reset_uicontrols(hObject,handles)
    ...
    set(handles.cb_plot_frequencies,'Value',0)
    set(handles.cb_joker1,'Value',0)
    set(handles.cb_joker2,'Value',0)
    set(handles.cb_plot_fitted,'Value',0)
    set(handles.cb_plot,'Value',0)
    set(handles.cb_flip,'Value',0)
...
%  flush previous data
    ...
    handles.pathname = handles.pathname(1:end-length('supersmith'));         
...
function cb_joker2_Callback(hObject, eventdata, handles)
...
function cb_joker1_Callback(hObject, eventdata, handles)
