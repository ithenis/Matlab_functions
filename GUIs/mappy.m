...
function pb_manual_crop_Callback(hObject, eventdata, handles)
    ...
    while go==1
        ...
        imagesc(handles.mydata.map(:,:,curr_map))
        axis equal
...
function pb_pop_out_fig_Callback(hObject, eventdata, handles)
    ...
    if get(handles.rb_surf,'Value')==1
        ...
        x = handles.mydata.x_map(1:step:end,1:step:end,curr_map)*10^3;
        y = handles.mydata.y_map(1:step:end,1:step:end,curr_map)*10^3;
        map = handles.mydata.map(1:step:end,1:step:end,curr_map)*10^6;
    ...
    elseif get(handles.rb_image,'Value')==1
        ...
        imagesc(x,  y  ,handles.mydata.map(:,:,curr_map)*10^6),   axis tight
        colormap(handles.colormap)
...
function rbp_horiz_vert_SelectionChangeFcn(hObject, eventdata, handles)
    ...
    if get(handles.rb_custom,'Value')
        set(handles.e_profile_width,'string',num2str(1));
        handles = pb_manual_profile_idx_Callback(hObject, eventdata, handles);
    else
        flag = 'direction';
        % call main profile function
        handles = create_profile( handles, hObject, flag); 
    end
...
% ************   manually pick crosshair in map axes   ************   
function pb_manual_crosshair_Callback(hObject, eventdata, handles)
    if strcmp(get(get(handles.rbp_horiz_vert,'SelectedObject'),'Tag'),'rb_custom')   
        disp('you can''t change crosshair while in ''custom'' mode')
        return
    end    
    axes(handles.axes1)
    [x,y] = ginput(1);    
...
function e_crosshair_x_Callback(hObject, eventdata, handles) 
    if strcmp(get(get(handles.rbp_horiz_vert,'SelectedObject'),'Tag'),'rb_custom')
        set(handles.e_crosshair_x,'String',num2str(handles.mydata.crosshair_xy(1)));
        disp('you can''t change crosshair while in ''custom'' mode')
        return
    end
    x = str2double(get(handles.e_crosshair_x,'String')); 
    x = round(x);
...
function e_crosshair_y_Callback(hObject, eventdata, handles)
    ...
    y = str2num(get(handles.e_crosshair_y,'String')); 
    y = round(y);
...
% ************   manually select the endpoints of the profile  ************    
...
function e_profile_x1_Callback(hObject, eventdata, handles)
    if strcmp(get(get(handles.rbp_horiz_vert,'SelectedObject'),'Tag'),'rb_vertical')
        set(handles.e_profile_x1,'String',num2str(handles.mydata.profile_xy(1,1)));
        return
    end   
    x1 = str2num(get(hObject,'string'));
    if x1>0 && x1<handles.mydata.profile_xy(2,1)
        handles.mydata.profile_xy(1,1) = x1;
    else
        set(handles.e_profile_x1,'String',num2str(handles.mydata.profile_xy(1,1)));
        return
    end
...
function e_profile_x2_Callback(hObject, eventdata, handles)
    ...
    if x2<size(handles.mydata.map,2) && x2>handles.mydata.profile_xy(1,1)
        handles.mydata.profile_xy(2,1) = x2;
    else
        set(handles.e_profile_x2,'String',num2str(handles.mydata.profile_xy(2,1)));
        return
    end
...
function e_profile_y1_Callback(hObject, eventdata, handles)
    ...
    if y1>0 && y1<handles.mydata.profile_xy(2,2)
        handles.mydata.profile_xy(1,2) = y1;
    else
        set(handles.e_profile_y1,'String',num2str(handles.mydata.profile_xy(1,2)));
        return
    end
...
function e_profile_y2_Callback(hObject, eventdata, handles)
    if strcmp(get(get(handles.rbp_horiz_vert,'SelectedObject'),'Tag'),'rb_horizontal')
        set(handles.e_profile_y2,'String',num2str(handles.mydata.profile_xy(2,1)));
        return
    end   
    y2 = str2num(get(hObject,'string'));
    if y2<size(handles.mydata.map,1) && y2>handles.mydata.profile_xy(1,2)
        handles.mydata.profile_xy(2,2) = y2;
    else
        set(handles.e_profile_y2,'String',num2str(handles.mydata.profile_xy(2,2)));
        return
    end
...
function pb_polyfit_Callback(hObject, eventdata, handles)
    ...
    fitcoef = polyfit(handles.profile.x, handles.profile.height, deg);
    handles.profile.fitpoly = polyval(fitcoef,handles.profile.x);
    handles.profile.fitted_height = handles.profile.height - handles.profile.fitpoly;
    handles.profile.fitted_slope = get_slope(handles.profile.fitted_height, handles.profile.x);
    
    figure(12)   , set(gcf, 'Color', 'w','Position',[50 50 600 700])
    subplot(2,1,1)
        plot(handles.profile.x*10^3, 10^6*handles.profile.height,...
             handles.profile.x*10^3, 10^6*handles.profile.fitted_height,'LineWidth',2)            
        ylabel('Figure [um]','Fontsize',12,'FontWeight','bold')
        xlim([handles.profile.x(1) handles.profile.x(end)]*10^3)
    subplot(2,1,2)
        plot(handles.profile.x*10^3, 10^6*handles.profile.phi,...
             handles.profile.x*10^3, 10^6*handles.profile.fitted_slope,'LineWidth',2)            
        ylabel('Slope [urad]','Fontsize',12,'FontWeight','bold')
        xlim([handles.profile.x(1) handles.profile.x(end)]*10^3)
...
% ************   extract profile    ************  
function handles = create_profile( handles, hObject, flag)
    
    [y,x,~] = size(handles.mydata.map);
    % if there's a change in profile direction, reset endpoints
    if exist('flag','var') && strcmp(flag,'direction')  % reset profile to new direction
        handles.profile = struct;
...
% create profile
    if strcmp(tag,'rb_horizontal')% horizontal line
        ...
        handles.profile.width = handles.mydata.dy*ny;
    elseif strcmp(tag,'rb_vertical')% vertical line
        ...
        handles.profile.width = handles.mydata.dx*ny;
    elseif strcmp(tag,'rb_custom')% random line
        [profPointsInd] = my_profile(handles.mydata.map, handles.mydata.profile_xy);
        [handles.profile.height, dix, diy] = my_profile(handles.mydata.map, handles.mydata.profile_xy);
        size(handles.profile.height)
        dx = dix*handles.mydata.dx;
        dy = diy*handles.mydata.dy;
        ...
        handles.profile.width = handles.profile.dx;
    end
    ...
    handles.profile.L = handles.profile.x(end)-handles.profile.x(1);
    handles.profile.L = round(handles.profile.L*10^5)/100;
    handles.profile.width = round(handles.profile.width*10^5)/100;
    set(handles.t_profile_width_mm,'String',['Width:     ' num2str(handles.profile.width) ' mm'])
    set(handles.t_profile_length_mm,'String',['Length:    ' num2str(handles.profile.L) ' mm'])
...
function handles = refresh_axes(handles)
    ...
    axes(handles.axes1)
        cla
        imagesc(handles.mydata.map(:,:,curr_map)*10^9),axis image,axis off
        colormap(handles.colormap)        
        hold on   
